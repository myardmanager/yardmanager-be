...
exports.getPart = async (req, res) => {
	try {
		const part = await Part.findOne({ _id: req.params.id, company: req.user.company });
		if (!part) {
...
exports.deletePart = async (req, res) => {
	try {
		const part = await Part.findOneAndDelete({ _id: req.params.id, company: req.user.company });
		if (!part) {
			return res.status(400).json({
				success: false,
				message: "Part not found"
