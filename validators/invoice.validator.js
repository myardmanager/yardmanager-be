...
exports.validateInvoice = [
	...
	check("datePaid").optional().isISO8601().toDate().withMessage("Date paid must be a valid date")
];
...
exports.validateInvoiceUpdate = [
	...
	check("products.*.quantity")
		...
		.isNumeric()
		.withMessage("Product quantity must be a number"),
	check("products.*.price").optional().isNumeric().withMessage("Product price must be a number"),
	check("products.*.date")
		.optional()
		.isISO8601()
		.toDate()
	...
	check("paid").optional().isNumeric().withMessage("Paid amount must be a number"),
